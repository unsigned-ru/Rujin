...
void Tron::Load()
{
	...
	{
		auto go = std::make_unique<GameObject>("StaticCube");

		//Render floor texture
		go->AddComponent(new BoxColliderComponent({ 50, 50 }));
		go->AddComponent<CollisionTestComponent>();

		//calc position
		go->GetTransform()->SetPosition({250, 400});
...

	{
		...
		//calc position
		go->GetTransform()->SetPosition({100, 400});
