...
namespace rujin
{
	...
	namespace input
	{
		...
		struct KeybindData
		{
			KeybindData(const ControllerButton controllerButton, const ButtonState triggerState, std::unique_ptr<command::Base>& pCommand);
			~KeybindData();
			...
			std::unique_ptr<command::Base> pCommand;
		};
		...
		public:
			...
			void RegisterCommand(ControllerButton button, std::unique_ptr<command::Base>&& command, ButtonState triggerState = ButtonState::Released) const;
