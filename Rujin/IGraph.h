...
namespace rujin::graph
{
	...
	public:
		...
		const std::vector<T_NodeType*>& GetAllNodes() const { return m_Nodes; }
		std::vector<T_NodeType*> GetAllActiveNodes() const;

		T_ConnectionType* GetConnection(int from, int to) const;
		const std::vector<std::list<T_ConnectionType*>>& GetAllConnections() const { return m_Connections; }
		const std::list<T_ConnectionType*>& GetNodeConnections(int idx) const;
		const std::list<T_ConnectionType*>& GetNodeConnections(T_NodeType* pNode) const { return GetNodeConnections(pNode->GetIndex()); }
	...
	protected:
		...
		std::vector<std::list<T_ConnectionType*>> m_Connections;
		std::vector<T_NodeType*> m_Nodes;
	...
	inline IGraph<T_NodeType, T_ConnectionType>::IGraph(const IGraph& other)
	{
		...
		{
			std::list<T_ConnectionType*> newList;
			for (auto c : cList)
	...
	inline int IGraph<T_NodeType, T_ConnectionType>::AddNode(T_NodeType* pNode)
	{
		...
		else
		{
			...
			m_Connections.push_back(std::list<T_ConnectionType*>());
